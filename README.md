...
![隔离策略详述](./images/img_13.png)

#### 1. 业务隔离
![业务隔离](./images/img_14.png)
业务隔离策略旨在通过区分不同业务场景的需求和限制条件，以更细粒度地控制流量和资源分配，确保秒杀活动的平稳运行。

- **B端（商家端）隔离**:
  - 商家或运营人员通过提报系统规划秒杀活动细节，包括商品信息、时间安排、库存量、限购规则、风控策略等。这种机制不仅帮助平台提前了解并规划资源需求，还能细化流量预期和风险控制点，为系统调整和优化提供依据。

- **C端（用户端）隔离**:
  - 通过预约机制收集用户参与意向，平台能基于预约数据预测流量峰值，从而做出适时的系统资源调配决策，比如动态扩容、设置流量阈值等，以应对高并发挑战。

#### 2. 系统隔离
![系统隔离](./images/img_15.png)
- **核心系统物理隔离**:
  - 针对秒杀活动中承受高流量冲击的核心系统（如订单处理、库存管理），采取物理隔离措施，独立部署，避免其与其他系统共享资源，减少相互影响，确保关键服务在高负载下的稳定运行。

- **非核心系统逻辑隔离**:
  - 对于交易链路末端或流量相对较小的系统，则可采用逻辑隔离方式，如使用不同的服务实例或通过配置路由规则，在软件层面实现隔离，提高资源使用的灵活性和效率。

#### 3. 数据隔离

- **秒杀商品数据隔离**:
  - 通过为参与秒杀的商品添加特定标签或标识，实现数据层面的隔离。这一步骤是确保秒杀请求能够被正确识别并引导至专设的微服务处理集群中的关键。数据隔离不仅能够减少对常规业务数据的影响，还能提升秒杀处理的效率和准确性，确保用户体验。

综上所述，隔离策略通过业务、系统、数据三个维度的精细划分与独立处理，构建了多层次的防护体系，有效管理和优化了秒杀活动期间的系统资源分配、流量控制及数据处理流程，为大规模并发场景下的系统稳定性提供了有力保障。


#### 预约系统架构设计

![预约系统架构设计](./images/img_16.png)


#### 限流总体流程

![限流总体流程](./images/img_17.png)