...
import io.check.seckill.stock.domain.model.enums.SeckillStockBucketArrangementMode;
...
public class SeckillStockBucketArrangementServiceImpl implements SeckillStockBucketArrangementService {
    ...
    public void arrangeStockBuckets(Long goodsId, Integer stock, Integer bucketsQuantity, Integer assignmentMode) {
        ...
        DistributedLock lock = distributedLockFactory
                .getDistributedLock(SeckillConstants.getKey(SeckillConstants.GOODS_STOCK_BUCKETS_SUSPEND_KEY,
    ...
    private List<SeckillStockBucket> buildBuckets(Long goodsId, Integer initStock, Integer availableStock,  Integer bucketsQuantity){
        if (goodsId == null || initStock == null || initStock <= 0 || availableStock == null || availableStock <= 0 ||
                bucketsQuantity == null || bucketsQuantity <= 0){
            throw new SeckillException(ErrorCode.PARAMS_INVALID);
        }
        ...
        for (int i = 0; i < bucketsQuantity - 1; i++) {
            SeckillStockBucket bucket = new SeckillStockBucket(goodsId, initAverageStocks, avaiableAverageStocks,
        ...
        SeckillStockBucket bucket = new SeckillStockBucket(goodsId, initRestStock, subRestStock > 0 ? initRestStock : availableRestStock,
    ...
    public SeckillStockBucketDTO getSeckillStockBucketDTO(Long goodsId, Long version) {
        ...
        SeckillBusinessCache<SeckillStockBucketDTO> seckillStockBucketCache = seckillStockBucketCacheService
                .getTotalStockBuckets(goodsId, version);
